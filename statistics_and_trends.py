...
def plot_statistical_plot(df):
    """
    Creates and saves a statistical plot using a corner plot to visualize
    pairwise relationships among numerical columns.
    """
    numerical_columns = [
    ...
    if existing_columns:
        ...
        print(f"Generating statistical plot for columns: {existing_columns}")
        ...
        fig.savefig('statistical_plot.png')
        plt.close(fig)
...
def writing(moments, col):
    ...
    if moments and all(m is not None for m in moments):
        ...
        skew_type = 'right-skewed' if skew > 0 else 'left-skewed' if skew < 0 else 'not skewed'
        kurtosis_type = (
